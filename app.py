...
def build_end_mill(t, mat_name, vc, fz):
    ...
    comp = ET.SubElement(comps, "Component", id=c_id, type="Cutter", subType="END MILL")
...
def build_bull_nose_mill(t, mat_name, vc, fz):
    ...
    comps = ET.SubElement(tool, "Components")
    comp = ET.SubElement(comps, "Component", id=c_id, type="Cutter", subType="BULL NOSE MILL")
    shape = ET.SubElement(comp, "Shape")
    ...
    with zipfile.ZipFile(zip_buffer, "w") as zf:
        for f in files:
            try:
                ...
                zf.writestr(f"{str(props['id']).replace(' ', '_')}.xml", xml_out)
                count += 1
